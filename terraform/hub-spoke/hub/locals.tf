locals {
  ...
  argocd_apps_all = {
    ...
    resources = var.argocd_files_config.load_resources ? file("${dirname(dirname(dirname(path.cwd)))}/bootstrap/hub/resources.yaml") : ""
    ...
    "workloads-resources-root" = var.argocd_files_config.load_resources && local.gitops_workloads_private_url == "" ? file("${dirname(dirname(dirname(path.cwd)))}/bootstrap/hub/workloads-resources-root.yaml") : ""
  }
